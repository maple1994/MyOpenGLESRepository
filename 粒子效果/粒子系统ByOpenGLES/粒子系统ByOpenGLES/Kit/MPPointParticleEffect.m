...
/// 设置粒子属性
- (void)setParticle:(MPParticleAttributes)aParticle
            atIndex:(NSUInteger)anIndex
{
    //mutableBytes:指向可变数据对象所包含数据的指针
    //获取粒子属性结构体内容
    MPParticleAttributes *particlesPtr = (MPParticleAttributes *)[self.particleAttributesData mutableBytes];
    //将粒子结构体对应的属性修改为新值
...
{
    MPParticleAttributes newParticle;
    ...
    {
        // 获取旧粒子
        MPParticleAttributes oldParticle = [self paricleAtIndex:i];
...
- (NSUInteger)numberOfParticles;
{
    static long last;
    //总数据/粒子结构体大小
    long ret = [self.particleAttributesData length] / sizeof(MPParticleAttributes);
...
- (void)prepareToDraw
{
    ...
    if (program != 0) {
         ...
         */
        glUniformMatrix4fv(uniforms[MPMVPMatrix], 1, 0,modelViewProjectionMatrix.m);
        // 一个纹理采样均匀变量
        /*
         ...
         */
        glUniform1i(uniforms[MPSamplers2D], 0);
        //粒子物理值
        //重力
        /*
         ...
         */
        glUniform3fv(uniforms[MPGravity], 1, self.gravity.v);
        //耗时
        glUniform1fv(uniforms[MPElapsedSeconds], 1, &elapsedSeconds);
        //粒子数据更新
        if(self.particleDataWasUpdated)
        {
            ...
            {  // 顶点属性没有送到GPU
                ...
                //数据大小
                GLsizeiptr size = sizeof(MPParticleAttributes);
                //个数
                int count = (int)[self.particleAttributesData length] /
                sizeof(MPParticleAttributes);
                
                self.particleAttributeBuffer =
                [[MPVertexAttribArrayBuffer alloc]
        ...
        // 准备顶点数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:MPParticleEmissionPosition
         numberOfCoordinates:3
         attribOffset:
         offsetof(MPParticleAttributes, emissionPosition)
         shouldEnable:YES];
        
        //准备粒子发射速度数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:MPParticleEmissionVelocity
         numberOfCoordinates:3
         attribOffset:
         offsetof(MPParticleAttributes, emissionVelocity)
         shouldEnable:YES];
        
        //准备重力数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:MPParticleEmissionForce
         numberOfCoordinates:3
         attribOffset:
         offsetof(MPParticleAttributes, emissionForce)
         shouldEnable:YES];
        
        //准备粒子size数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:MPParticleSize
         numberOfCoordinates:2
         attribOffset:
         offsetof(MPParticleAttributes, size)
         shouldEnable:YES];
        
        //准备粒子的持续时间和渐隐时间数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:MPParticleEmissionTimeAndLine
         numberOfCoordinates:2
         attribOffset:
         offsetof(MPParticleAttributes, emissionTimeAndLife)
         shouldEnable:YES];
...
- (BOOL)loadShaders
{
    ...
    vertShaderPathname = [[NSBundle mainBundle] pathForResource:
                          @"MPPointParticleShader" ofType:@"vsh"];
    ...
    fragShaderPathname = [[NSBundle mainBundle] pathForResource:
                          @"MPPointParticleShader" ofType:@"fsh"];
    ...
    //位置
    glBindAttribLocation(program, MPParticleEmissionPosition,
                         "a_emissionPosition");
    //速度
    glBindAttribLocation(program, MPParticleEmissionVelocity,
                         "a_emissionVelocity");
    //重力
    glBindAttribLocation(program, MPParticleEmissionForce,
                         "a_emissionForce");
    //大小
    glBindAttribLocation(program, MPParticleSize,
                         "a_size");
    //持续时间、渐隐时间
    glBindAttribLocation(program, MPParticleEmissionTimeAndLine,
    ...
    // 获取uniform变量的位置.
    //MVP变换矩阵
    uniforms[MPMVPMatrix] = glGetUniformLocation(program,"u_mvpMatrix");
    //纹理
    uniforms[MPSamplers2D] = glGetUniformLocation(program,"u_samplers2D");
    //重力
    uniforms[MPGravity] = glGetUniformLocation(program,"u_gravity");
    //持续时间、渐隐时间
    uniforms[MPElapsedSeconds] = glGetUniformLocation(program,"u_elapsedSeconds");
...
//默认重力
const GLKVector3 MPDefaultGravity = {0.0f, -9.80665f, 0.0f};
